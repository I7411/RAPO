"""
RAPO-STH-ARO-POA: Stagnation-triggered Hybridization
=======================================================

Cơ chế lai:
- ARO chạy chính trên toàn bộ quần thể.
- Nếu best fitness không cải thiện đáng kể sau `stagnation_patience` vòng lặp,
  kích hoạt POA escape để xử lý một nhóm cá thể xấu nhất.
- POA escape chạy trong `escape_iterations` vòng phụ, dùng Phase 1 Predator Attack
  và Phase 2 Defense Mechanism để tăng đa dạng và thoát bẫy cục bộ.
...
Gợi ý đặt file:
metaheuristic_benchmark/algorithms/hybrids/rapo_stagnation_triggered_aro_poa.py
"""
...
class RAPOStagnationTriggeredAROPOA:
    """
    ARO-main + POA-escape khi trì trệ.
    ...
    - stagnation_patience: số vòng không cải thiện đáng kể trước khi kích hoạt POA.
    - improvement_epsilon: ngưỡng cải thiện đáng kể, dùng cho điều kiện:
        new_best < old_best - improvement_epsilon
    - escape_iterations: số vòng phụ L cho POA escape.
    - escape_ratio: tỷ lệ cá thể xấu nhất được POA xử lý, ví dụ 0.30 = 30%.
    - max_evaluations: ngân sách đánh giá hàm mục tiêu, dùng để so sánh công bằng.
    """

    name = "RAPO_STH_ARO_POA"
    ...
    def optimize(self, objective_func: ObjectiveFunc, dimension: int) -> OptimizationResult:
        ...
        for t in range(1, self.max_iterations + 1):
            ...
            # 1) Chạy tối ưu chính bằng ARO.
            for i in range(self.population_size):
                if self._budget_exhausted:
                    break
                candidate = self._aro_candidate(population, i, t, lb, ub)
            ...
            # 3) Nếu trì trệ đủ lâu, kích hoạt POA escape.
            poa_escape_updates = 0
            triggered = False
                ...
                trigger_events.append(
                    {
                        ...
                        "updates": int(poa_escape_updates),
                    }
                )
                stagnation_count = 0
            ...
            iteration_trace.append(
                {
                    ...
                    "aro_updates": int(aro_updates),
                    "poa_escape_triggered": triggered,
                    "poa_escape_updates": int(poa_escape_updates),
                    "nfe": int(self.nfe),
                }
            )
    ...
    def _aro_detour_foraging(self, population: np.ndarray, i: int) -> np.ndarray:
        ...
        # Running length / running operator bản vector hóa.
        progress = 0.0  # detour dùng như một bước nhiễu độc lập ở đây
        length = (math.e - math.exp(progress)) * math.sin(2.0 * math.pi * r2)
        c = np.zeros(d)
...
if __name__ == "__main__":
    ...
    optimizer = RAPOStagnationTriggeredAROPOA(
        population_size=40,
        max_iterations=100,
        lower_bound=-100,
        upper_bound=100,
