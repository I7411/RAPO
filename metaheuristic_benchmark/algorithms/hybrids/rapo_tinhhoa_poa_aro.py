"""
RAPO Elite-guided Hybridization: POA -> ARO
================================================
...
Hybrid mechanism:
    ...
    POA is used as the main optimizer to create promising elites.
    After every M iterations, the top-k POA elites are exploited or expanded
    by ARO operators for L micro-iterations.
...
class RAPOEliteGuidedPOAARO:
    """
    RAPO-EGH variant: POA creates elites, ARO refines/explores around POA elites.

    Main loop:
        1. Evolve the whole population using POA Phase 1 + Phase 2.
        2. Save global best.
        3. If t mod M == 0, select top-k elites from the POA population.
        4. For each elite, run ARO-guided local update for L micro-steps.
    ...
    refinement_interval:
        M. Apply ARO refinement every M iterations.
    local_iterations:
        L. Number of ARO micro-iterations for each selected elite.
    seed:
    ...
    def optimize(self, objective_func: ObjectiveFunc, dimension: int) -> OptimizationResult:
        """Run RAPO-EGH POA -> ARO on a minimization problem."""
        if dimension < 1:
        ...
        for t in range(1, self.max_iterations + 1):
            ...
            # Main POA evolution.
            poa_phase1_improvements = 0
            poa_phase2_improvements = 0

            for i in range(self.population_size):
                if self._should_stop():
                    break

                # POA Phase 1: Predator Attack towards better pufferfish.
                candidate = self._poa_attack(population, fitness, i)
                if candidate is not None:
                    ...
                    if candidate_fit <= fitness[i]:
                        ...
                        poa_phase1_improvements += 1

                if self._should_stop():
                    break

                # POA Phase 2: Defense Mechanism.
                candidate = self._poa_defense(population[i], t, lb, ub)
                candidate = self._repair(candidate, lb, ub)
                candidate_fit = self._evaluate(objective_func, candidate)
                if candidate_fit <= fitness[i]:
                    population[i] = candidate
                    fitness[i] = candidate_fit
                    poa_phase2_improvements += 1

                if fitness[i] < best_fitness:
                    best_fitness = float(fitness[i])
                    best_solution = population[i].copy()

            # Elite-guided ARO local update around top-k POA elites.
            refined_count = 0
            if not self._should_stop() and t % self.refinement_interval == 0:
                event_before = best_fitness
                refined_count = self._aro_refine_top_elites(
                ...
                refinement_events.append(
                    {
                        "iteration": t,
                        "mode": "POA_elite_to_ARO_refinement",
            ...
            iteration_trace.append(
                {
                    ...
                    "nfe": self.nfe,
                    "poa_phase1_improvements": poa_phase1_improvements,
                    "poa_phase2_improvements": poa_phase2_improvements,
                    "aro_refined_elites": refined_count,
                }
            )
        ...
        return OptimizationResult(
            ...
            metadata={
                "algorithm": "RAPO_EGH_POA_ARO",
                "hybrid_type": "Elite-guided Hybridization",
                "main_optimizer": "POA",
                "elite_refiner": "ARO_Detour_or_Random_Hiding",
    ...
    def _poa_attack(self, population: Array, fitness: Array, i: int) -> Optional[Array]:
        """POA Phase 1: choose a better candidate pufferfish and attack it."""
        ...
        interaction_factor = self.rng.integers(1, 3, size=population.shape[1])
        return population[i] + self.rng.random(population.shape[1]) * (selected_pufferfish - interaction_factor * population[i])
    ...
    def _aro_refine_top_elites(
        ...
        """Refine top-k POA elites using ARO operators around elite points."""
        ...
        for idx in elite_indices:
            ...
            for micro_t in range(1, self.local_iterations + 1):
                ...
                effective_t = min(self.max_iterations, max(1, outer_iteration + micro_t))
                energy = self._aro_energy(effective_t)
                if energy > 1.0:
                    candidate = self._aro_detour_around_elite(current, population, effective_t, lb, ub)
                else:
                    candidate = self._aro_hiding_around_elite(current, effective_t, lb, ub)
    ...
    def _aro_detour_around_elite(self, elite: Array, population: Array, t: int, lb: Array, ub: Array) -> Array:
        dim = elite.size
        j = int(self.rng.integers(0, population.shape[0]))
        anchor = population[j]
        ...
        # Elite-guided detour: move from a random anchor toward the elite region.
        return anchor + running_operator * (elite - anchor) + perturb_flag * noise

    def _aro_hiding_around_elite(self, elite: Array, t: int, lb: Array, ub: Array) -> Array:
        dim = elite.size
        ...
        burrow = elite + hiding_parameter * mask * elite
        running_operator = self._aro_running_operator(dim, t)
        return elite + running_operator * (float(self.rng.random()) * burrow - elite)
...
if __name__ == "__main__":
    ...
    optimizer = RAPOEliteGuidedPOAARO(
        population_size=50,
        max_iterations=100,
        lower_bound=-100,
        upper_bound=100,
