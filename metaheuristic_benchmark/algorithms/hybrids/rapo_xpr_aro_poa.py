...
class RAPOExploitationReplacementAROPOA(_RAPOBase):
    """RAPO-XPR ARO->POA.

    Low-level Relay / Exploitation Phase Replacement.
    Keep the ARO control framework and ARO Energy Shrink.
    Replace ARO Random Hiding with Pufferfish-POA Phase 2 Defense Mechanism.

    If A(t) > energy_threshold: keep ARO Detour Foraging.
    Else: use POA Defense Mechanism.
    """
    ...
    def optimize(
        ...
        operator_counter = {"aro_detour_foraging": 0, "poa_defense_mechanism": 0}
            ...
            for t in range(1, self.max_iterations + 1):
                for i in range(self.population_size):
                    ...
                    if energy > self.energy_threshold:
                        candidate = self._aro_detour_foraging(i, population, t)
                        operator_counter["aro_detour_foraging"] += 1
                    else:
                        candidate = self._poa_defense_mechanism(i, population, t)
                        operator_counter["poa_defense_mechanism"] += 1
        ...
        return OptimizationResult(
            ...
            metadata={
                "algorithm": "RAPO_XPR_ARO_POA",
                "hybrid_type": "Low-level Relay / Exploitation Phase Replacement",
                "framework": "ARO",
                "kept_operator": "ARO Detour Foraging",
                "replaced_phase": "ARO Random Hiding",
                "replacement_operator": "POA Phase 2 - Defense Mechanism",
...
if __name__ == "__main__":
    ...
    optimizer = RAPOExploitationReplacementAROPOA(
        population_size=30,
        max_iterations=80,
        lower_bound=-100.0,
        upper_bound=100.0,
