"""
rapo_sequential_poa_aro.py

RAPO Sequential Hybrid Reverse: POA -> ARO
================================================

Cơ chế lai:
    - 50% số vòng lặp đầu chạy POA-Pufferfish.
    - 50% số vòng lặp sau chạy ARO.
    - Đây là lai tuần tự cấp cao theo thời gian:
        High-level Relay / Sequential Hybrid.
    - Quần thể, fitness và nghiệm tốt nhất được kế thừa liên tục.
    - Không khởi tạo lại quần thể khi chuyển từ POA sang ARO.
...
Gợi ý đặt file trong framework:
    metaheuristic_benchmark/algorithms/hybrids/rapo_sequential_poa_aro.py
...
class RAPOSequentialPOAARO:
    """
    RAPO Sequential Hybrid Reverse: POA -> ARO.

    Luồng lai:
        Giai đoạn 1: POA-Pufferfish chạy trước.
        Giai đoạn 2: ARO chạy sau.

    Nếu switch_ratio = 0.5:
        50% vòng lặp đầu dùng POA.
        50% vòng lặp sau dùng ARO.
    ...
    Ghi chú công bằng so sánh:
        ...
        Vì vậy, nếu so sánh nghiêm ngặt với thuật toán khác, nên dùng max_evaluations
        để đồng bộ ngân sách đánh giá hàm mục tiêu NFE.
    """
    ...
    def optimize(
        ...
        """
        Chạy RAPO Sequential POA -> ARO trên một hàm mục tiêu.
        ...
        try:
            ...
            # Nếu budget quá nhỏ, các cá thể chưa đánh giá sẽ bị gán inf.
            fitness[evaluated_count:] = np.inf
            if evaluated_count > 0:
                population[evaluated_count:] = population[0]
            else:
                population[:] = self.rng.uniform(lb, ub, size=(self.population_size, dimension))
        ...
        # Với max_iterations = 100, switch_ratio = 0.5:
        # switch_iteration = 50, tức t=1..50 dùng POA, t=51..100 dùng ARO.
        ...
        return OptimizationResult(
            algorithm="RAPO_Sequential_POA_ARO",
            ...
            metadata={
                ...
                "minimization": True,
                "hybrid_type": "High-level Relay / Sequential Hybrid",
    ...
    def _select_phase_algorithm(self, iteration: int, switch_iteration: int) -> str:
        return "POA" if iteration <= switch_iteration else "ARO"

    def _phase_rule_text(self, switch_iteration: int) -> str:
        return (
            f"POA, 1 <= t <= {switch_iteration}; "
            f"ARO, {switch_iteration + 1} <= t <= {self.max_iterations}"
        )
...
if __name__ == "__main__":
    optimizer = RAPOSequentialPOAARO(
        population_size=30,
        max_iterations=100,
        lower_bound=-100.0,
        upper_bound=100.0,
        switch_ratio=0.5,       # 50% đầu POA, 50% sau ARO
        seed=42,
